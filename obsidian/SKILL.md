...
The user keeps two Obsidian vaults as plain markdown files synced via iCloud:
one personal, one work. Each vault is just a directory tree, so all standard
Unix tools work — no Obsidian app required to read or write notes.
...
Because these paths contain spaces and tildes, **always quote them** in shell
commands. A useful shorthand is to assign the relevant one once per session:
...
**Picking which vault:** if the user says "work vault" or "personal vault",
use that one directly. Otherwise infer from content — company/product/customer
topics (e.g. Solo products, work meetings, `Customers/`) go in `work`;
personal-life topics (e.g. investing, personal errands) go in `personal`. When
genuinely ambiguous, check both (`fd`/`rg` accept multiple paths) before
picking one, rather than defaulting to whichever path happens to be documented
first.

If neither `$VAULT` exists (e.g. on a non-mac host or before iCloud has
synced), stop and tell the user — don't fabricate notes.
...
This layout is shared structure common to both vaults. Each vault also has its
own topic-specific folders at the top level — e.g. `personal` has `Investing/`,
`work` has `Solo Products/`, `Customers/`, `Technical Fundamentals/`. Don't
assume one vault's topic folders exist in the other; `fd -t d . "$VAULT" -d 1`
